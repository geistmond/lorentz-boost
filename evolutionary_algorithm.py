...
def ackley_new2(x, y, z):
    """
    Adds a Z term with tangent function in place of scalar.
    Tries to limit dimensions to 2D for surface plotting of (x,y).
    """
    return (np.exp(-0.2 * np.sqrt(0.5 * (x**2. + y**2.))) - np.exp(0.5 * (np.cos(2. * np.pi * x) + np.cos(2. * np.pi * y))) + np.e) / np.tan(-2. * np.pi * z)
...
def es_comma(objective: Callable[[float,float], float], 
    ...
    """
    Es Comma algorithm is a graph search that optimizes functions. It is used in evolutionary methods.
    It can be used to quantize and optimize any callable function with given bounds.
    The idea is to see if it is more optimal to apply Lorenz boosts and conservation laws to QM wave packets.
    If it is more optimal according to tools for optimizing functions, that implies favored by thermodynamics.
    Experimental testing would need to follow.
    """
...
def plot_function(r_min, r_max)->None:
    ...
    x_axis = np.linspace(r_min, r_max, 8000)
    #y_axis = np.arange(r_min, r_max, 0.1)
    y_axis = np.linspace(r_min, r_max, 8000)
